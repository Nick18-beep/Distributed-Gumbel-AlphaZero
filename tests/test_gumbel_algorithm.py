...
def _network_apply(observation):
    return NetworkOutput(
        policy_logits=jnp.zeros((observation.shape[0], 7), dtype=jnp.float32),
...
def test_unknown_algorithm_fails_before_claiming_registration() -> None:
    ...
    with pytest.raises(KeyError, match="unknown algorithm"):
        create_algorithm(config, game=game, search_backend=MctxSearchBackend())
...
def test_gumbel_algorithm_select_action_is_deterministic_and_masks_illegal() -> None:
    ...
    game = create_game(config.game.name)
    algorithm = create_algorithm(config, game=game, search_backend=MctxSearchBackend())
    state = game.init()
    ...
    assert jnp.isclose(jnp.sum(output_a.policy_target), 1.0)
...
def test_gumbel_algorithm_generates_value_target_from_to_play_perspective() -> None:
    ...
    game = create_game(config.game.name)
    algorithm = create_algorithm(config, game=game, search_backend=MctxSearchBackend())
    observation = jnp.zeros(game.observation_shape, dtype=jnp.float32)
    legal = jnp.ones((game.num_actions,), dtype=bool)
    policy = jnp.ones((game.num_actions,), dtype=jnp.float32) / game.num_actions
    trajectory = Trajectory(
        game_id="g",
        game_name=game.name,
        algorithm_name=algorithm.name,
        model_version=0,
        final_rewards=jnp.asarray([1.0, -1.0], dtype=jnp.float32),
        steps=[
